...
def get_category_embeddings(embeddings_metadata):
    ...
    if embedding_model == "glove":
        model_id = embeddings_metadata["model_type"]  #
    else:
        model_id = embeddings_metadata.get("model_name", "default")

    cache_key = f"cat_embed_{embedding_model}_{model_id}"
    ...
    for category in categories:
        ...
        if embedding_model == "openai":
            st.session_state[cache_key][category] = get_openai_embeddings(category, model_name=model_id)
        elif embedding_model == "glove":
            st.session_state[cache_key][category] = get_glove_embeddings(
                category,
                embeddings_metadata["word_index_dict"],
                embeddings_metadata["embeddings"],
                embeddings_metadata["model_type"]
            )
        else:
            st.session_state[cache_key][category] = get_sentence_transformer_embeddings(category, model_name=model_id)
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    for word in words:
        # Get embedding for the single word
        word_vec = get_glove_embeddings(word, word_index_dict, embeddings, model_type)

        # If the word exists (not a zero vector), add it to the sum
        if np.any(word_vec):
    ...
    return sentence_vec  # Returns zero vector if no words were found
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embedding_model == "glove":
        ...
        input_vec = averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type)
        cache_key = "cat_embed_glove_" + model_type

    elif embedding_model == "openai":
        model_name = embeddings_metadata["model_name"]
        input_vec = get_openai_embeddings(sentence, model_name=model_name)
        cache_key = "cat_embed_openai_" + model_name

    else:
        model_name = embeddings_metadata["model_name"]
        input_vec = get_sentence_transformer_embeddings(sentence, model_name=model_name)
    ...
    similarity_scores = []
    category_vectors = st.session_state[cache_key]
